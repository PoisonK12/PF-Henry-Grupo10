    ...
    <div className={style.infoAvailable} >
        ...
        <div className={`d-flex ${style.container}`}>
            <div className={`d-flex flex-column ${style.container1}`}>
                    <legend className={style.legend}> Complete los siguientes datos</legend>
                    <fieldset className={style.fieldsetContainer}>
                        ...
                        <div className={`d-flex flex-column ${style.inputContainer}` }>
                            ...
                            <div className="m-2 d-flex flex-column form-group  text-start">
                                ...
                                <input  type="text" className={`form-control ${style.input}`} name="guestName" value={form.guestName} onChange={handleChange} placeholder="Escriba el nombre del inquilino"></input>
                                </div>
                            <div className="m-2 d-flex flex-column form-group text-start">
                                <label>Numero del teléfono</label>
                                <input type="text"  className={`form-control ${style.input}`} name="guestPhoneNumber" value={form.guestPhoneNumber} onChange={handleChange} placeholder="Escriba el núm de telefono"></input>
                            </div>
                            <div className="m-2 d-flex flex-row form-group text-start">
                                <div>
                                    <label htmlFor="caldendar1" className="form-label">Fecha de ingreso : </label>
                                    <input  id="caldendar1" type="date" className={`form-control ${style.input}`} value={form.checkInDate} disabled></input>
                                </div>
                                <div>                           
                                    <label htmlFor="calendar2" className="form-label">Fecha de salida :</label>
                                    <input id="calendar2"  type="date" className={`form-control ${style.input}`} value={form.checkOutDate} disabled></input>
                                </div>
                            </div>
                            <div className="m-2 d-flex flex-column form-group text-start">
                                <span> Elige una forma de pago : </span>
                                <select name="paymentMethod"  className={`form-control ${style.input}`} value={form.paymentMethod} onChange={handleChange}>
                            ...
                            <div className="col-md-12 " style={{marginTop : "20px" , marginBottom : "30px"}}>
                                ...
                                <button class={`btn ${style.button}`} style={{marginLeft : "10px"}} type="button" onClick={(e) => handlePayment(e)} disabled={payment}> 
                                        <svg className={style.svg} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"  width="40" height="30" fill="#ffffff">
                            ...
                            <div >
                                ...
                                <span> Acepto los <Link to="/terms&Conditions" className={style.link}>términos y condiciones</Link></span>
                            </div> 
                        </div>
                    </fieldset> 
            </div>
            <div className={`d-flex flex-column ${style.container2} `}>
                ...
                <div className={ `d-flex flex-row  justify-content-between ${style.child}`}>
                ...
                <div className={ `d-flex flex-row  justify-content-between ${style.child}`}>
                    ...
                    <select style={{background : "transparent"}}>{[1,2,3,4,5].map(guest => {
                ...
                <p>$ {assetData.rentPrice + ( 21 * assetData.rentPrice) / 100 }</p>
                 ...
                 <div style={{width : "400px" , margin : "40px 0"}}>
                    {!paymentSucess ?  (
                        ...
                        <p style={{
                          ...
                          }}> Gracias por confiar en nosotros ! <p>Les deseamos un feliz hospedaje 💚</p></p>
                     ) }
                    </div>
        <div>
           <div className={style.buttons}>
            <div>
                <button type="button"  class={`btn ${style.button}`} onClick={handleBack}>↩ Volver</button>
            </div>
            <div>
                <button type="submit" class={`btn ${style.button}`}  disabled={buttonReserv}> 📑 Reservar </button>
            </div>
            </div>
               </div>
